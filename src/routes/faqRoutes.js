...
const tableName = TABLE.FAQ;
const ine_faqs_ModuleID = TABLE.FAQ_MODULE_ID;

// Add
router.post('/', async (req, res) => {
    try {
        const { title, description, apihitid } = req.body;

        // Validate request data
        if (!title) {
            return sendResponse(res, { error: 'Title fields are required', status: false }, 400);
        }

        // Insertion
        const [insertResult] = await pool.query(`INSERT INTO ${tableName} (title, description,created_by) VALUES (?, ?, ?)`, [
            title,
            description,
            apihitid,
        ]);
...
router.get('/', async (req, res) => {
    try {
        ...
        if (id) {
            const [results] = await pool.query(`SELECT * FROM ine_faqs WHERE status = 1 and id = ?`, [id]);
            if (results.length > 0) {
        ...
        const [results] = await pool.query(`SELECT * FROM ine_faqs WHERE status = 1 order by id desc`);
...
router.delete('/', async (req, res) => {
    try {
        ...
        await Promise.all(deletedIds.map(async (deletedId) => {
            ...
            activityLog(ine_faqs_ModuleID, currentRecord, null, 3, 0);
        }));
