...
const tableName = TABLE.WAREHOUSE_WAREHOUSE;
...
router.post('/', async (req, res) => {
    try {
        ...
        const { name } = req.body;
        if (!name) { return sendResponse(res, { error: 'Name fields are required', status: false }, 400); }
        await pool.query(`INSERT INTO ${tableName} (name) VALUES (?)`, [name]);
...
router.put('/', async (req, res) => {
    try {
        ...
        const { name } = req.body;
        await pool.query(`UPDATE ${tableName} SET name = ?, updated_at = NOW() WHERE id = ?`, [name, id]);
