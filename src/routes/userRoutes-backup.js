...
const { getQueryParamId, getRecordById, ManageResponseStatus, sendResponse, getQueryParamIds, activityLog, generateOTP, validatePassword, checkEmailExistOrNot, checkPhoneExistOrNot, processImageUpload, isValidDate } = require('../commonFunctions')
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (rows.length > 0) {
      ...
      if (passwordMatch) {
        const token = jwt.sign({ data: user }, API_SECRET_KEY, { expiresIn: API_TOKEN_EXPIRESIN });
        return res.status(200).json({ data: token, message: 'Login Successfully', status: true });
      } else {
...
router.post('/', async (req, res) => {
  try {
    const { first_name, last_name, email, phone, password, role_id, address, state, district, pincode, govt_id_number, pan_number, govt_id_upload, pan_upload, referral_code, avatar } = req.body;

    // Validate request data
    if (!first_name || !last_name || !email || !password || !role_id) {
      return sendResponse(res, { error: 'Role, First Name, Last Name, Email and Password field is required', status: false }, 400);
    }
    ...
    const govt_id_upload1 = ''; // await processDocuments(govt_id_upload); // Government Document
    const pan_upload1 = ''; // await processDocuments(pan_upload); // Pancard Document
    const avatar1 = ''; // await processDocuments(avatar); // Avatar Document

    // Generate unique referral code
    const uniqueCode = uniqueId.slice(0, 8); // Generate a short UUID
    const referralCode = `${first_name}${last_name}${uniqueCode}`; // Combine first name, last name, and unique code

    // Insertion
    const [insertResult] = await pool.query(`INSERT INTO ${tableName} (role_id, first_name, last_name, email, phone, password, avatar, prefix_id) VALUES (?,?,?,?,?,?,?,?)`, [
      role_id, first_name, last_name, email, phone, hashedPassword, avatar, newPrefix
    ]);

    const insertedRecordId = insertResult.insertId;

    // User Details - Insertion
    await pool.query(`INSERT INTO ${tableName2} (user_id, address, state, district, pincode, govt_id_number, pan_number, govt_id_upload, pan_upload, my_referral_code) VALUES (?,?,?,?,?,?,?,?,?,?)`, [
      insertedRecordId, address, state, district, pincode, govt_id_number, pan_number, govt_id_upload1, pan_upload1, referralCode,
    ]);
    ...
    let referId = null;
    if (referral_code) {
      const [referralUser] = await pool.query(`SELECT id FROM ${tableName2} WHERE my_referral_code = ? LIMIT 1`, [referral_code]);
      if (referralUser) {
...
router.put('/', async (req, res) => {
  try {
    ...
    const { role_id, first_name, last_name, email, phone, password, gender, govt_id_number, pan_number, govt_id_upload, pan_upload, avatar, date_of_birth, anniversary, status } = req.body;
    ...
    if (govt_id_upload) {
      if (govt_id_upload?.imageData)
        gimg = await processImageUpload(govt_id_upload);
      else {
        gimg = govt_id_upload;
      }
    }
    if (pan_upload) {
      if (pan_upload?.imageData)
        pimg = await processImageUpload(pan_upload);
      else {
        gimg = pan_upload;
      }
    }
    if (avatar) {
      if (avatar?.imageData)
        aimg = await processImageUpload(avatar);
      else {
        aimg = avatar;
      }
    }
    ...
    await pool.query(`UPDATE ${tableName2} SET date_of_birth = ?, anniversary = ?, gender = ?, govt_id_number = ?, pan_number = ?, govt_id_upload = ?, pan_upload = ? WHERE user_id = ?`, [date_of_birth, anniversary, gender, govt_id_number, pan_number, gimg, pimg, id]);
...
router.put('/changepassword', async (req, res) => {
  try {
    ...
    // Check if the ID exists in the database and retrieve the existing record
    const [existingRecordResults] = await getRecordById(id, tableName, 'id');
    if (existingRecordResults.length === 0) {
      return sendResponse(res, { error: ManageResponseStatus('notFound'), status: false }, 404);
    }
