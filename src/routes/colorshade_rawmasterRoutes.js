// src/routes/colorshade_rawmasterRoutes.js
...
const tableName = TABLE.COLORSHADE;
const ine_color_shade_ModuleID = TABLE.COLORSHADE_MODULE_ID;
...
router.post('/', async (req, res) => {
    try {
        ...
        const [insertResult] = await pool.query(`INSERT INTO ${tableName} (name, description, created_by) VALUES (?, ?, ?)`, [
            ...
            apihitid || null, // Use null for optional fields
        ]);

        const insertedRecordId = insertResult.insertId;
        const insertedRecord = await getRecordById(insertedRecordId, tableName, 'id'); // Retrieve the inserted record

        await activityLog(ine_color_shade_ModuleID, null, insertedRecord, 1, 0); // Maintain Activity Log
...
router.put('/', async (req, res) => {
    try {
        ...
        await activityLog(ine_color_shade_ModuleID, existingRecord, updatedRecord, 2, 0); // Maintain Activity Log
...
router.delete('/', async (req, res) => {
    try {
        ...
        await Promise.all(deletedIds.map(async (deletedId) => {
            ...
            activityLog(ine_color_shade_ModuleID, currentRecord, null, 3, 0);
        }));
